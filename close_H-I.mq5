...
//---------------------------------------------------------------------------------------------------------
//--- plot close_pm                        
#property indicator_label1  "close_pm"     // Nombre 
...
input int inpt_ma = 20;


//---------------------------------------------------------------------------------------------------------
double   close_pmBuffer[];
...
int OnInit(){
   SetIndexBuffer(0,close_pmBuffer,INDICATOR_DATA);
...
int OnCalculate(const int rates_total, const int prev_calculated, const datetime &time[],
   ...
   for(int i=star; i<rates_total; i++){
      if(i > 0){
         if(MathAbs(high[i] - low[i]) >  MathAbs(high[i-1] - low[i-1])){
            close_pmBuffer[i] = (high[i-1] - low[i-1]) + ((high[i] - low[i])/2); 
         }else{
            close_pmBuffer[i] = (high[i] - low[i]) + ((high[i-1] - low[i-1])/2);
         } 
      }    
   }
   SimpleMAOnBuffer(rates_total,star, 0,inpt_ma,close_pmBuffer,ma_close_vBuffer);
   return(rates_total);
}
